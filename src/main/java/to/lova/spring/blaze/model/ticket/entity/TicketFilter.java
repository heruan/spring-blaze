...
import to.lova.spring.blaze.model.customer.entity.AbstractCustomer_;
import to.lova.spring.blaze.model.customer.entity.Customer_;
import to.lova.spring.blaze.model.customer.entity.ShippingAddress;
import to.lova.spring.blaze.model.customer.entity.ShippingAddress_;
...
public class TicketFilter implements Specification<Ticket>, Serializable {
    ...
    public Predicate toPredicate(Root<Ticket> root, CriteriaQuery<?> query,
        ...
        if (!Strings.isNullOrEmpty(this.customer)) {
            var customerPath = root.get(Ticket_.customer);
            var customerIdPath = customerPath.get(AbstractCustomer_.id);
            var customerPredicate = criteriaBuilder.equal(customerIdPath,
                    this.customer);
            if (this.includeShippingAddresses) {
                var customerType = root.type();
                var addressCustomerIdPath = criteriaBuilder
        ...
        if (this.createdFrom != null) {
            var path = root.get(Ticket_.creationInstant);
        ...
        if (this.author != null) {
            var path = root.get(Ticket_.author).get(User_.id);
            predicates.add(criteriaBuilder.equal(path, this.author));
        }

        if (this.assignee != null) {
            var path = root.get(Ticket_.assignee).get(User_.id);
